...
def get_val_timelet(filename,t_slots,ANS_START_T, ANS_END_T,TIMELET_INV):
    ...
    try:
        ...
        print 'error in loading ', filename,' ... Skip and need to be purged from sensor list'
    ...
    stime_read=[[] for i in range(len(t_slots))] # Creat the list of lists for seconds index
    utc_t=[];val=[]
...
def time_range_check(input_files, ANS_START_T, ANS_END_T, TIMELET_INV):
    ...
    t_range_min = dt.datetime(2000, 1, 1, 0, 0, 0).replace(tzinfo=tz.gettz('UTC')).astimezone(pytz.timezone('US/Pacific'))
    t_range_max = dt.datetime(2100, 1, 1, 0, 0, 0).replace(tzinfo=tz.gettz('UTC')).astimezone(pytz.timezone('US/Pacific'))
    purge_list = []
    ...
    for i, filename in enumerate(input_files):
        try:
            ...
            data = dill_load_obj(filename)

            if not (len(data["ts"]) and len(data["value"])):
                print 'Not enough data included: the number of samples is zero'
                purge_list.append(filename)

            elif len(data["ts"]) < est_num_time_slot or \
                 len(data["value"]) < est_num_time_slot:
...
def get_val_timelet(filename,t_slots,ANS_START_T, ANS_END_T,TIMELET_INV):
    ...
    sensor_read=[[] for i in range(len(t_slots))] # Create the list of lists for value
    stime_read=[[] for i in range(len(t_slots))] # Create the list of lists for seconds index
    utc_t=[];val=[]
    for t_sample, v_sample in zip(time_val,sensor_val):
...
###############################################################################
"""
def get_weather_timelet(data_dict,t_slots,TIMELET_INV):
    ...
    for date_idx,date in enumerate(daterange(t_start, t_end, inclusive=True)):
        ...
        for h_idx,hour_sample in enumerate(data_day):
            ...
            if (h_idx==0) and (date_idx==0):
                ...
                cur_sec_val=cur_minute_val*MIN
                ...
                try:# time slot index a given weather sample time
                    ...
                    for sample_idx,each_sample in enumerate(hour_samples):
                        ...
                        if sensor_name in data_dict:
                            if each_sample!='N/A' and each_sample!=[]:
                                ...
                                data_dict[sensor_name][0][vtt_dt_idx].append(cur_sec_val)
                    ...
                    vtt_dt_idx=-1
            else: # hour_sample is list of weather filed name, dget_weather_timeletiscard
                hour_sample=[]
    ...
    for date_idx,date in enumerate(daterange(t_start, t_end, inclusive=True)):
        ...
        for h_idx,hour_sample in enumerate(data_day):
            ...
            if (h_idx==0) and (date_idx==0):
                ...
                # convert to offset-naive from offset-aware datetimes
                vtt_dt=dt.datetime(*(vtt_dt_aware.timetuple()[:6])).replace(tzinfo=tz.gettz('UTC')).astimezone(pytz.timezone('US/Pacific'))
...
"""    
def get_weather_timelet(data_dict,t_slots,TIMELET_INV,USE_WEATHER_DATA_BIN=False):
    print 'getting weater data new ' 
    print '------------------------------------'
    t_start=t_slots[0] 
    t_end=t_slots[-1]
...
def construct_data_dict(input_files, ANS_START_T,ANS_END_T,TIMELET_INV, include_weather=1,binfilename='data_dict',IS_USING_PARALLEL=False):
    ...
    if IS_SAVING_DATA_DICT:
        ...
        #end__dictproc_t=time.time()
        #print 'the time of saving data dict.bin is ', end__dictproc_t-start__dictproc_t, ' sec'
...
def foo_2(tup):
    ...
    uuid = temp[:temp.rindex('.')]
    try:
        if os.path.isfile(uuid+FL_EXT) and mt.loadObjectBinaryFast(uuid+FL+EXT):
            return
    except:
        pass
    dict_sensor_val, dict_stime,utc_t,val=get_val_timelet(file_name,time_slots,ANS_START_T, ANS_END_T, TIMELET_INV)
    ...
    if IS_SAVING_INDIVIDUAL:
        mt.saveObjectBinaryFast(ret[1],str(ret[0]) + '.bin')
...
def data_dict_purge(purge_list,data_dict):
...
def construct_data_dict_2(input_files, ANS_START_T,ANS_END_T,TIMELET_INV, include_weather=1,binfilename='data_dict',IS_USING_PARALLEL=False):
    # Variable Declare and initialization
    time_slots=[]
    start=ANS_START_T
    while start < ANS_END_T:
    ...
    if IS_USING_PARALLEL:
        ...
        p.join()
        #sensor_list = data_dict.keys()
        sensor_list = [i.split('/')[-1][:-len_fl_ext] for i in input_files]
        #print_report(start_time)
    ...
    if include_weather==1:
        ...
        #12 WindDirDegrees
        #13 DateUTC
        weather_list=get_weather_timelet(data_dict,time_slots,TIMELET_INV,USE_WEATHER_DATA_BIN=True)
        # Convert symbols to Integer representaion
        data_dict['Conditions'][1],Conditions_dict=symbol_to_state(data_dict['Conditions'][1])
        data_dict['Events'][1],Events_dict=symbol_to_state(data_dict['Events'][1])
        data_dict.update({'sensor_list':sensor_list})
        ...
        for key_id in weather_list:
            ...
            data_dict[key_id]=temp_list
            if IS_SAVING_INDIVIDUAL:
                mt.saveObjectBinaryFast(data_dict[key_id],str(key_id)+'.bin')
    # use stored bin file
    elif include_weather==2:
        print 'use weather_dict.bin'
        # This part to be filled with Khiem......
    ...
    if IS_SAVING_INDIVIDUAL:
        mt.saveObjectBinaryFast(time_slots,'time_slots.bin')
        mt.saveObjectBinaryFast(sensor_list,'sensor_list.bin')
        mt.saveObjectBinaryFast(weather_list,'weather_list.bin')
        mt.saveObjectBinaryFast(Conditions_dict,'Conditions_dict.bin')
        mt.saveObjectBinaryFast(Events_dict,'Events_dict.bin')
        mt.saveObjectBinaryFast(data_dict['Conditions'],'Conditions.bin')
        mt.saveObjectBinaryFast(data_dict['Events'],'Events.bin')
    ...
    if IS_SAVING_DATA_DICT:
        ...
        #end__dictproc_t=time.time()
        #print 'the time of saving data dict.bin is ', end__dictproc_t-start__dictproc_t, ' sec'
        sizeoutput = int(shlex.split(subprocess.check_output("stat -c %s data_dict.bin", shell=True))[0])
        print 'Saved bin file size is ', round(sizeoutput/10**6,2), 'Mbyte'
    return data_dict        
