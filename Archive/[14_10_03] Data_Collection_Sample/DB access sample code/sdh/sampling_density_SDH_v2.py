...
def compute_sampling_density_matrix(start_time, end_time, delta_t, max_sr):
    ...
    for uuid in uuid_list:
        ...
        if min_sampling_density == 0.0 and max_sampling_density == 0.0:
    ...
    ### Plot min, max, avg. sorted sensor accodring to avg ###
    ...
    plt.legend(loc='best')
    pl.savefig('SDH_sampling_density_stats.png', dpi=100, bbox_inches='tight')    
    #plt.show()        
...
def visualize_density_matrix(sampling_density_matrix):
    ...
    pl.savefig('SDH_sampling_density_map.png',dpi=100, bbox_inches='tight')        
    #plt.show()
...
if __name__ == "__main__":
    ...
    delta_t = 3600000 ### 3600000 ms = 1hr
    if len(sys_argv) == 5:        
    ...
    sampling_density_matrix = np.array(sampling_density_matrix)
    saveObjectBinary(sampling_density_matrix, 'SDH_sampling_density_matrix.obj')
    saveObjectBinary(good_list, 'SDH_good_list.obj')
    saveObjectBinary(broken_list, 'SDH_broken_list.obj')
