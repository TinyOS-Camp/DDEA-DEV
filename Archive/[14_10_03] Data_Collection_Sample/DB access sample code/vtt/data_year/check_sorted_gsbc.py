...
def load_gsbc_list():
    with open('gsbc_uuids.bin','r') as f:
        lines = f.readlines()
...
def check_sorted(uuid):
    ...
    PATH = '/data/bigdata/gsbc/Binfiles_raw/'
...
def estimate_frequency(uuid):
    ...
    if len(ts) <= 1:
        print uuid + '\t<= 1 sample'
        return
    diff_list = np.array([(ts[i+1][0]-ts[i][0]).seconds for i in range(len(ts)-1)])
    avg = np.mean(diff_list)
    print uuid + '\t' + str(avg)
   
if __name__ == '__main__': 
    uuid_list = mt.loadObjectBinaryFast('gsbc_uuids.bin')
     #print uuid_list[0]
     #merge_gvalley(uuid_list[0])

    p = Pool(12)
    #p.map(merge_gvalley,uuid_list)
    p.map(estimate_frequency,uuid_list)
    p.close()
    p.join()
