...
def cluster_measurement_points(m_matrix,m_name,corr_bnd=[0.1,0.9],alg='pack'):
    exemplars_dict={}    
...
##############################################################################
"""
def cluster_report(X_INPUT,input_names):
    n_labels = labels.max()
    print '============================================================'
    print 'num of sensors: ', X_INPUT.shape[1] ,' , ',  'num of cluasters: ',len(exemplars)    
    print 'exemplars: ',input_names[exemplars]
    print '============================================================'
    nb_avg_err=[]
    for i in range(n_labels + 1):
        print '------------------------------------------------------------------------------'
        print('Cluster %i: Exemplar -  %s' % ((i + 1), input_names[exemplars[i]]))
        print(' %s' % ', '.join(input_names[labels == i]))
        nb_set=list(set(np.nonzero(labels == i)[0])-set([exemplars[i]]))
        nb_err=np.mean(DIST_MAT[exemplars[i],nb_set])
        print 'average error' , nb_err
        nb_avg_err.append(nb_err)
        
    print '============================================================'
    print 'Exclueded sensor list for Anaylsis due to zero variance'
    print '============================================================'
    print zero_var_list


def find_sd_adjusted_norm_dist_matrix(X_INPUT,SD):
    ...
    for i in range(X_INPUT.shape[1]):
        for j in range(X_INPUT.shape[1]):
            wgt_ij=SD[:,i]*SD[:,j] # weigth coefficient
            # sample density normalize sample 1
            sample1=X_INPUT[:,i].copy();
            inf_idx=np.nonzero(sample1==np.inf)
            noinf_idx=np.nonzero(sample1!=np.inf)
            sample1[inf_idx]=np.mean(sample1[noinf_idx])
            sample1=wgt_ij*sample1;sample1=sample1/norm(sample1)
            # sample density normalize sample 2        
            sample2=X_INPUT[:,j].copy();
            inf_idx=np.nonzero(sample2==np.inf)
            noinf_idx=np.nonzero(sample2!=np.inf)
            sample2[inf_idx]=np.mean(sample2[noinf_idx])
            sample2=wgt_ij*sample2; sample2=sample2/norm(sample2)
...
def plot_cluster_corr(DIST_MAT,input_names,m_labels, m_exemplars,cluster_id):
    horizontalalignment = 'left'
    verticalalignment = 'top'
    n_labels=int(m_labels.max()+1)
    if m_labels.max()<cluster_id:
        raise NameError('the cluster id: '+ str(cluster_id) +'  is not in cluster ids')
    plt.figure('cluster correlation trend')
    nb_set=np.nonzero([m_labels == cluster_id])[1]
    plt.plot(nb_set,DIST_MAT[m_exemplars[cluster_id],nb_set],'-r*',label='clusters')
    plt.plot(range(DIST_MAT.shape[0]),DIST_MAT[m_exemplars[cluster_id],:],'s',label='samples')
    plt.xlabel('sensor id'); plt.ylabel('distance')
    plt.legend();plt.title('For cluster '+input_names[m_exemplars[cluster_id]])
    for nb_idx in nb_set:
        name=input_names[nb_idx]
        x=nb_idx;y=DIST_MAT[m_exemplars[cluster_id],nb_idx]
        plt.text(x+0.-02, y+0.02, name, size=8,
                horizontalalignment=horizontalalignment,
                verticalalignment=verticalalignment,
                bbox=dict(facecolor='w',
                          edgecolor=plt.cm.spectral(nb_idx / float(n_labels)),
                          alpha=.6))
