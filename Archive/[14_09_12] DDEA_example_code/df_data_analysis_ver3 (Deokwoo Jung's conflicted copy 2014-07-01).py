...
if BN_ANALYSIS==1:
    ...
    # 1. Analysis of  'GW2.CG_SYSTEM_ACTIVE_POWER_M'
    #Power profile from Apr 2013- Apr 2014
    PLOT_ALL=0
    if PLOT_ALL==1:
        for s_name_ in all_psensors:
            print 'loading ',s_name_,'....'
            filename=DATA_DIR+s_name_+FL_EXT
            data = mt.loadObjectBinary(filename)
            val_= data["value"]
            time_val = data["ts"]
            dtime_=[time_val_[0] for time_val_ in time_val]
            title('Power profile of '+s_name_+' from Apr 2013- Apr 2014',fontsize=20)
            figure()    
            plot(dtime_,val_)
            plt.xlabel('Time',fontsize=18)
            plt.ylabel('Power',fontsize=18)
            plt.tick_params(labelsize='large')
            plt.get_current_fig_manager().window.showMaximized()
            plt.savefig(fig_dir+s_name_+'(APR 2013-APR 2014).png')
            time.sleep(1.5)
    ...
    def weather_convert(wdata_mat,wdata_name, Conditions_dict,Events_dict):
        ...
        dewp_idx=wdata_name.index('Dew PointC')
    ...
    def bldg_obj_weather_convert(bldg_obj):
        # For avg
        wdata_mat = bldg_obj.avgdata_weather_mat.copy()
        wdata_name =bldg_obj.avgweather_names
        ...
        bldg_obj.avgweather_dict=weather_dict
        bldg_obj.avgdata_weather_mat_=wdata_mat
        # For diff
        wdata_mat = bldg_obj.diffdata_weather_mat.copy()
        wdata_name =bldg_obj.diffweather_names
        Conditions_dict= bldg_obj.Conditions_dict.copy()
        Events_dict= bldg_obj.Events_dict.copy()
        wdata_mat,weather_dict=weather_convert(wdata_mat,wdata_name, Conditions_dict,Events_dict)
        bldg_obj.diffweather_dict=weather_dict
        bldg_obj.diffdata_weather_mat_=wdata_mat
